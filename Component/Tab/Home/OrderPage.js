...
import { foodImage, Back, Diner, Multiply } from "../ImportImage";
import {headerStyle} from './HomeTab';
const { width, height } = Dimensions.get("window");
const OrderPage = ({ navigation, navigation: { navigate } }) => {
  const [selected, setSelected] = useState("v1");
  const [overlay, setOverlay] = useState(false);
  return (
    <Container>
      {overlay && <Thanks props={navigate} />}
      <Header style={headerStyle.header}>
        <Left style={headerStyle.headerLeft}>
          <Button style={headerStyle.headerBack} transparent onPress={() => navigation.goBack()}>
            <Back fill='#ccc' width={24} height={18}/>
          </Button>
        </Left>
        <Body style={headerStyle.headerBody}>
          <Title style={headerStyle.headertitle}>청미래 닭갈비</Title>
        </Body>
        <Right>
          <Diner fill="#c2c2c2" width={22} height={22}/>
        </Right>
      </Header>
      <Content>
        <Card>
          <CardItem>
            ...
            <Body style={{ paddingLeft: 10, alignSelf: "center" }}>
              ...
              >
                ...
                <Text>{navigation.getParam("price")}원</Text>
              </Text>
            </Body>
          </CardItem>
        </Card>
        <List>
          ...
          <ListItem itemHeader>
            <Text>{navigation.getParam("price")}원 결제</Text>
          </ListItem>
        </List>
        <Form>
          <Picker
...
const Thanks = ({ props }) => {
  ...
  return (
    ...
    >
      ...
      <Button
        transparent
        onPress={() => props("OrderList", setActiveIndex(3))}
        style={{ marginHorizontal: "auto", marginVertical: "auto" }}
      >
        <View style={{position:'relative', top:10, left:-10}}>
        <Text style={{color:'#9c48fc',position:'absolute', width:150, zIndex:99, top:-72, left:-20, fontWeight:'bold'}}>감사합니다!</Text>
        <Thumbnail style={{width:128, height:128,top:-100, left:-50, borderRadius:80, position:'absolute'}} source={require('../../../assets/icon/OrderComplete.jpg')}/>
        <Multiply style={{backgroundColor:'#fff',top:14 ,borderRadius:50, position:'absolute'}} fill="#9c48fc" />
        </View>
      </Button>
    </View>
  );
